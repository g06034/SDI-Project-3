...
function phoneNumberPattern(phoneNumberStr)
{
  ...
  if (tokens[0].length==3) // check the length of the first substring (area code) should be 3 characters
  {
    ...
    {
      if(isNaN(parseInt(tokens[1]))) // and its a not a number add the errors otherwise go to the next validation
        countOfErrors++;
    }
    else
    {
    ...
    {
      countOfErrors++; // if final segment is not 4 chars long add an error to the ecounter
    }
